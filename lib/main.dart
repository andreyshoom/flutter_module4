      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: nav.map((name) {
            if (name.toString() == 'Мои фото') {
              return ListView(key: PageStorageKey(name), children: [
                for (var item in data['Мои фото']) Image.network(item),
              ]);
            } else {
              return ListView(key: PageStorageKey(name), children: [
                for (var item in data['Галерея']) Image.network(item),
              ]);
            }
          }).toList(),
        ),
